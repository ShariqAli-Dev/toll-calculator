...
func main() {
	listenAddr := flag.String("listenAdr", ":6000", "the listen address of the HTTP server")
	aggregatorServiceAddr := flag.String("aggServiceAddr", "http://localhost:3001", "the listen address of the aggregator service")
	flag.Parse()
	...
	logrus.Infof("gateway HTTP server running on port %s\n", *listenAddr)
	log.Fatal(http.ListenAndServe(*listenAddr, nil))
}

func (h *InvoiceHandler) handleGetInvoice(w http.ResponseWriter, r *http.Request) error {
	invoice, err := h.client.GetInvoice(context.Background(), 5)
	if err != nil {
		return err
	}
